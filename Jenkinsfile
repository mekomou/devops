  ...
  if(env.BRANCH_NAME ==~ /release.*/){
        ...
        stage('Release Build And Upload Artifacts') {
         ...
         stage("Deploy from Artifactory to QA"){
           retrieveArtifact = 'http://localhost:8081/artifactory/libs-release-local/com/mekomou/devops/' + artifactVersion + '/devops-' + artifactVersion + '.war'
           echo "${tagVersion} with artifact version ${artifactVersion}"
           echo "Deploying war from http://localhost:8081/artifactory/libs-release-local/com/mekomou/devops/${artifactVersion}/devops-${artifactVersion}.war"
